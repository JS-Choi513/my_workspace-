...
### PR #33 — 리뷰 반영 + 테스트 + CI 수정

**리뷰(claude[bot]) 지적 2건 수정**:
1. `api/routers/jobs.py` — `create_job()`에서 `hw_manual_checks` 누락으로 Phase 1 데이터 항상 NULL 저장되던 버그 수정
2. `checks/profiles/gpu_server.json` — `smartctl` → `smartmontools` (apt 패키지명 오류)
3. `.claude/rules/profiles.md` — 문서 동기화

**테스트 추가 (3건)**:
- `test_jobs.py` — `hw_manual_checks` DB 저장 검증
- `test_sw_install.py` — `grub_only_reboot_triggered` 조건 4케이스 parametrize
- `test_validate.py` — retry 중 `_mark_failed` 미호출 / max_retries 시 호출 검증

**CI 수정**:
- `pyproject.toml` dev deps에 `aiosqlite` 추가 (누락 시 test_api 전체 실패)
