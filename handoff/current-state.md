...
- `.claude/rules/error-handling.md` 신규 작성 → **PR #11 생성 (머지 대기)**
  ...
  - 에러 로그 경로·포맷 (JSONL, task별 분리, `./logs/{job_id}/{task}.jsonl`)
  - 민감정보 마스킹 규칙 (structlog processor)
- `context/target-servers.md` 신규 작성 → **자동커밋으로 main 반영 완료**
  - ManyCoreSoft / DeepGadget 브랜드 소개
  - 제품 라인업 (dg5W/dg5R/dg5W-TT 현행, dg3S/dg3R/dg4W/dg4R/dg4F 단종 RMA)
  - 제품별 스펙 (마더보드 포함: ASUS WRX90E-SAGE SE, ASRock Rack GNR2D32G-2L+ 등)
  - 탑재 가속기 목록 (NVIDIA 주력, Tenstorrent n300s/p150a, AMD, FuriosaAI)
  - 검수 환경 (SSH 직접 접근, 기본 계정 deepgadget/deepgadget, OS 혼재)
...
**추천 시작점:** `.claude/rules/migration.md` 작성

이유: target-servers.md 완료. 다음 우선순위.

### 남은 문서 작업 (우선순위 순)

| 순서 | 문서 | 상태 |
|------|------|------|
| 1 | `.claude/rules/migration.md` | 미작성, 다음 세션 시작점 |
| 2 | `context/infra-environment.md` | 미작성 |
| 3 | 필수 시스템 설정 목록 문서 | 미작성 (E 항목에서 파생) |

### migration.md 작성 전 물어봐야 할 것

- Alembic 마이그레이션 실패 시 롤백 정책 (자동 rollback? 수동?)
- staging 환경 존재 여부 (migration 검증 절차)
- zero-downtime migration 요건 여부 (서비스 중단 허용?)
- 마이그레이션 적용 시점 (배포 전? 배포 후? 동시?)
