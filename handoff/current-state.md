...
| #12 | docs/migration-rules | 머지 대기 (리뷰 반영: worker_sw_install 제거, DB 접속정보 수정) |
| #13 | docs/sys-config-rules | 머지 대기 (리뷰 반영: GPU power/clock 조건부, is-enabled 루프 수정) |
| #14 | fix/critical-c1-c3-c7 | 머지 대기 (리뷰 반영: worker_sw_install 제거, build→migration 순서 수정) |
...
4. **CRITICAL C-7** `deploy.sh` 마이그레이션 단계 삽입
5. 연기된 항목(C-2/C-4/C-5/C-6) TODO 주석으로 코드에 마킹
6. PR #14 생성 (`fix/critical-c1-c3-c7`)
